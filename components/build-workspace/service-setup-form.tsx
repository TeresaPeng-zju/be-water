            ...
            <fieldset>
              ...
              <div className="mt-3 flex flex-wrap gap-2">
                {serviceTypes.map((type) => (
                  <button key={type} type="button" onClick={() => { setValue("serviceType", type, { shouldValidate: true }); if (!getValues("name")) setValue("name", shortcutNames[type], { shouldValidate: true }); }} className={cn("service-type-chip relative overflow-hidden rounded-full border px-3.5 py-2 text-sm outline-none transition duration-200 focus-visible:ring-4 focus-visible:ring-[var(--focus)]", serviceType === type ? "service-type-chip--active -translate-y-0.5 border-[var(--brand)] bg-[var(--brand-soft)] text-[var(--brand-dark)] shadow-[0_6px_16px_rgba(33,111,100,0.12)]" : "border-[var(--line-strong)] bg-white text-[var(--muted)] hover:-translate-y-0.5 hover:border-[#aebbb5] hover:text-[var(--ink)] hover:shadow-sm")} aria-pressed={serviceType === type}>{type}</button>
                ))}
              </div>
            </fieldset>
